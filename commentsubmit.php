...
$EMAIL_ADDRESS = "ewshopfive@gmail.com";
...
// Direktori absolut tempat folder _comments berada di server
// Contoh: /home/user/public_html/_comments/
$COMMENTS_DIR = "/home/ava/tropicyou.github.io/_comments";

// Ekstensi file komentar
$FILE_EXT = ".yaml";
...
// Buat nama file unik berdasarkan timestamp dan post_id
$filename = date("Y-m-d-H-i-s") . "-" . md5($post_id) . $FILE_EXT;
$filepath = $COMMENTS_DIR . $filename;

// Tulis konten YAML ke file
if (file_put_contents($filepath, $msg)) {
    // Opsional: tetap kirim email notifikasi
    mail($EMAIL_ADDRESS, $SUBJECT, "Komentar baru disimpan di $filepath", "From: $EMAIL_ADDRESS");
    include $COMMENT_RECEIVED;
} else {
    echo "Gagal menyimpan komentar. Pastikan folder _comments bisa ditulis (chmod 755 atau 777).";
}
{
	include $COMMENT_RECEIVED;
}
